...
class CrossAttention(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        k1, v1 = (
            self.kv1(x1).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).contiguous()
        )
        k2, v2 = (
            self.kv2(x2).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).contiguous()
        )
