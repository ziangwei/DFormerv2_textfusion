...
with Engine(custom_parser=parser) as engine:
    try:
        ...
        # Validation can use larger batch size (no backward, saves memory)
        # Typically 4-8x training batch is safe
        val_dl_factor = 4

        # SUNRGBD: increase from gpus*1 to gpus*16 for ~4x speedup
        # Other datasets: use 4x training batch
        if config.dataset_name == "SUNRGBD":
            val_batch_size = int(args.gpus * 16)  # More aggressive for SUNRGBD
